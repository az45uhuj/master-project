...
def make_model():
    ...
    resnet.load_state_dict(torch.load('E:\RAF_landmarks\\mix_resnet34.pt'))
    resnet.cuda()
    ...
    for race in races:
        pre_gender = pd.DataFrame(
            columns=['Image','p_left_eye_centerx', 'p_left_eye_centery', 'p_right_eye_centerx', 'p_right_eye_centery', 'p_nose_tipx',
                     'p_nose_tipy', 'p_mouth_left_cornerx', 'p_mouth_left_cornery','p_mouth_right_cornerx', 'p_mouth_right_cornery'])
        filepath = 'F:\master thesis\\face dataset\RAF\\basic\\' + race + '_test.csv'
        imgpath = 'F:\master thesis\\face dataset\RAF\\basic\Image\original\\'
