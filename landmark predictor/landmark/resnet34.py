...
train, val = train_test_split(datasets.landmarks_dataframe, test_size=0.2, random_state=42)

transform_objs = transforms.Compose([datasets.ToPILImage(), datasets.ToTensor()])
...
img_dir = '/media/qi/Elements/3d_morphable_model_199/'
train_dataset = datasets.CustomImageDataset(train, img_dir, transform_objs)
val_dataset = datasets.CustomImageDataset(val,  img_dir, transform_objs)
...
def train():
    ...
    for epoch in range(20):
        ...
        if val_epoch_loss < best_loss:
            ...
            #torch.save(resnet.state_dict(), 'E:\\3d_morphable_model_199\\resnet34\\' +'balanced_resnet.pt')
...
def visulize(trainLoss, valLoss):
    ...
    plt.xlabel('epoch')
    plt.legend()
    #plt.savefig('F:\VGG16_Race_Classifier\\resnet/loss.png')
    plt.show()
    ...
    with torch.no_grad():
      for sample in dataloader:
        ...
        for j in range(inputs.size()[0]):
            ...
            ax.imshow(img)
            ax.set_title('{:.3f}'.format(((lms_gt-lms)**2).mean().item()))
            ax.axis('off')
            ...
            if images_so_far == num_images:
                ...
                #plt.savefig('E:\\3d_morphable_model_199\\resnet34\\balanced_example.png', bbox_inches='tight')
                plt.show()
                return
        model.train(mode=was_training)
        plt.show()
